...
|8     |`two_stage`|WARL      | When 0, S-stage only translation is supported
...
|15:12 |`Rv64_Mode`|WARL      | Maximum virtual-memory size addressing supported in Rv64: +
                                Value 0: No virtual-memory system supported +
                                Value 1-7: Reserved for standard use +
                                Value 8: Sv39 +
                                Value 9: Sv48 +
                                Value 10: Sv57 +
                                Value 11-13: reserved for standard use +
                                Value 14-15: reserved for custom use
...
When `two_stage` is 0, `Rv32_Mode` and `Rv64_Mode` give the information of
which paging schemes are supported in single stage translation. For instance,
if `Rv64_Mode` indicates Sv48, it means that Sv48 paging scheme is supported,
it implies that Sv39 is also supported.

When `two_stage` is 1, `Rv32_Mode` and `Rv64_Mode` give the information of
which paging schemes are supported. For instance, if `Rv64_Mode` indicates
Sv48, it means that Sv48 paging scheme is supported, it implies that Sv39 is
also supported for S/VS-stage translation and that Sv48x4 and Sv39x4 are
supported for G-stage translation.
...
control of the G-stage page tables and clear the `two_stage` field of the 
