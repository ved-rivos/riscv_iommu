...
When `ENSA` is 1, the `IOSUM` (permit Supervisor User Memory access) bit 
modifies the privilege with which supervisor privilege transactions access 
virtual memory. When `IOSUM=0`, superviosr privilege transactions to pages 
mapped with U-bit in PTE set to 1 will fault.

When `ENSA` is 1, supervisor privilege trasactions that read with execute 
